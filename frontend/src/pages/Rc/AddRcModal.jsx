...
const AddRcModal = ({ isOpen, onClose, onSubmit, initialExtractionFile }) => {
  ...
  useEffect(() => {
    if (!isOpen) {
      setFormData({ vehicleNumber: '', chassisNo: '', engineNo: '', make: '', model: '' })
      if (rcFrontPreview?.startsWith('blob:')) URL.revokeObjectURL(rcFrontPreview)
      if (rcBackPreview?.startsWith('blob:')) URL.revokeObjectURL(rcBackPreview)
      setRcFrontFile(null)
      setRcBackFile(null)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/rc`, submitData, { withCredentials: true })
      if (onSubmit) onSubmit(submitData)
      onClose()
    } catch (error) {
      console.error(error)
      alert(error.response?.data?.message || 'Failed to create RC record')
    } finally {
      ...
      <div className='bg-white rounded-xl md:rounded-2xl shadow-2xl max-w-2xl w-full max-h-[95vh] overflow-hidden flex flex-col'>
        <div className='bg-gradient-to-r from-orange-600 to-amber-600 p-3 md:p-4 text-white flex-shrink-0'>
          <div className='flex justify-between items-center'>
            <div>
              <h2 className='text-lg md:text-2xl font-bold'>Add RC</h2>
              <p className='text-orange-100 text-xs md:text-sm mt-1'>Add a new vehicle registration certificate</p>
            </div>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col flex-1 overflow-hidden'>
          ...
          <div className='border-t border-gray-200 p-3 md:p-4 bg-gray-50 flex flex-col md:flex-row justify-end items-center gap-3 flex-shrink-0'>
            <div className='flex gap-2 md:gap-3 w-full md:w-auto'>
              ...
              <button type='submit' disabled={isSubmitting} className='flex-1 md:flex-none px-6 md:px-8 py-2 bg-gradient-to-r from-orange-600 to-amber-600 text-white rounded-lg hover:shadow-lg font-semibold transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed'>
                {isSubmitting ? 'Saving...' : 'Save RC'}
              </button>
            </div>
          </div>
        </form>
