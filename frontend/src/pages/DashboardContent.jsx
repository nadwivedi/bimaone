      ...
      <main className='w-full space-y-5 px-3 pt-4 pb-32 md:space-y-6 lg:px-8 lg:pt-6 lg:pb-10'>
        ...
        <section className='overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200'>
          <div className='bg-gradient-to-r from-[#1f2a3c] via-[#27374f] to-[#314866] px-4 py-3 text-white md:px-6'>
            <h1 className='text-lg font-bold md:text-xl'>Upload Insurance Policy</h1>
            <p className='text-xs text-slate-300 md:text-sm'>Drop the policy and AI will fill in all the details for you</p>
          </div>
          ...
          <div className='p-3 md:p-5'>
            <div
              ...
              className={`group flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed px-4 py-6 text-center transition-all md:py-7 ${
                isDragOver
                  ? 'scale-[1.01] border-blue-500 bg-blue-50'
                  : 'border-blue-200 bg-gradient-to-r from-blue-50 to-sky-50 hover:border-blue-400'
              }`}
            >
              <span className={`flex h-12 w-12 items-center justify-center rounded-2xl text-white shadow-lg shadow-blue-700/20 transition-transform md:h-14 md:w-14 ${isDragOver ? 'scale-110 bg-blue-600' : 'bg-gradient-to-br from-blue-700 to-blue-500 group-hover:scale-105'}`}>
                <Svg d={ICON.cloud} className='h-6 w-6 md:h-7 md:w-7' />
              </span>
              <p className='mt-3 text-base font-bold text-slate-800 md:text-lg'>
        ...
        <section className='overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200'>
          ...
          {loadingDocs ? (
            ...
            <>
                      ...
                      <tr key={`${d.type}-${d.id}`} onClick={() => setViewingDoc(d)} className='cursor-pointer transition hover:bg-slate-50'>
                        <td className='px-6 py-3.5'>
                          <div className='flex items-center gap-3'>
                            <span className={`flex h-10 w-10 items-center justify-center rounded-xl ring-1 ring-inset ${t.tile}`}>
                  ...
                  return (
                    <li key={`${d.type}-${d.id}`}>
                      <button type='button' onClick={() => setViewingDoc(d)} className='flex w-full items-center gap-3 px-4 py-3 text-left active:bg-slate-50'>
                        <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ring-1 ring-inset ${t.tile}`}>
                          <Svg d={t.icon} className='h-5 w-5' />
                        </span>
              ...
              <p className='border-t border-slate-100 px-4 py-3 text-xs text-slate-500 md:px-6'>
