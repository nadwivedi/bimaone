...
const EditTaxModal = ({ isOpen, onClose, onSubmit, tax }) => {
      ...
      if (parts.length === 3) {
        ...
        if (!isNaN(day) && !isNaN(month) && !isNaN(year) && year > 1900) {
          ...
          if (!isNaN(taxFromDate.getTime())) {
            ...
            const monthsToAdd = { 'Q1': 3, 'Q2': 6, 'Q3': 9, 'Q4': 12 }
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (showVehicleDropdown && vehicleMatches.length > 0) {
        if (e.key === 'ArrowDown') {
          e.preventDefault()
          setSelectedDropdownIndex(prev => prev < vehicleMatches.length - 1 ? prev + 1 : 0)
          return
        }
        if (e.key === 'ArrowUp') {
          e.preventDefault()
          setSelectedDropdownIndex(prev => prev > 0 ? prev - 1 : vehicleMatches.length - 1)
          return
        }
        if (e.key === 'Enter') {
          e.preventDefault()
  ...
  const handleManualDocumentUpload = (e) => {
    ...
    if (file.type === 'application/pdf' || file.type.startsWith('image/')) {
      ...
      setUploadedTaxDocument(prev => {
        if (prev?.previewUrl) URL.revokeObjectURL(prev.previewUrl)
        return { name: file.name, type: file.type === 'application/pdf' ? 'pdf' : 'image', previewUrl: URL.createObjectURL(file) }
      })
    ...
    try {
      ...
      reader.onloadend = async () => {
        try {
          ...
          const response = await axios.post(`${API_URL}/api/ocr/tax`, { imageBase64: base64String }, { withCredentials: true })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`${API_URL}/api/tax/${tax._id}`, dataToSubmit, { withCredentials: true })
      if (response.data.success) {
  ...
  return (
    <div className='fixed inset-0 bg-black/60 z-[80] flex items-center justify-center p-2 md:p-4'>
      <div className='bg-white rounded-xl md:rounded-2xl shadow-2xl max-w-5xl w-full max-h-[80vh] md:max-h-[95vh] overflow-hidden flex flex-col'>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col flex-1 overflow-hidden'>
                          ...
                          >
                            <div className='flex justify-between items-start'>
                              <div>
                                ...
                                <p className='text-xs text-gray-700 mt-1'>{vehicle.ownerName || 'N/A'}</p>
                                {vehicle.chassisNumber && (
                                  <p className='text-xs text-gray-500 mt-0.5'>Chassis: {vehicle.chassisNumber}</p>
                                )}
                              </div>
                              <svg className={`w-5 h-5 ${index === selectedDropdownIndex ? 'text-indigo-600' : 'text-indigo-400'}`} fill='none' stroke='currentColor' viewBox='0 0 24 24'>
              ...
              <div className='mb-4'>
                ...
                <div className='grid grid-cols-4 gap-2'>
                  <button type='button' onClick={() => setTaxPeriod('Q1')} className={`px-3 py-2 rounded-lg font-semibold text-sm transition-all cursor-pointer ${taxPeriod === 'Q1' ? 'bg-purple-600 text-white shadow-lg ring-2 ring-purple-300' : 'bg-white text-gray-700 border-2 border-purple-200 hover:border-purple-400'}`}>
                    Q1<span className='block text-[10px] font-normal mt-0.5'>3 Months</span>
                  </button>
                  <button type='button' onClick={() => setTaxPeriod('Q2')} className={`px-3 py-2 rounded-lg font-semibold text-sm transition-all cursor-pointer ${taxPeriod === 'Q2' ? 'bg-purple-600 text-white shadow-lg ring-2 ring-purple-300' : 'bg-white text-gray-700 border-2 border-purple-200 hover:border-purple-400'}`}>
                    Q2<span className='block text-[10px] font-normal mt-0.5'>6 Months</span>
                  </button>
                  <button type='button' onClick={() => setTaxPeriod('Q3')} className={`px-3 py-2 rounded-lg font-semibold text-sm transition-all cursor-pointer ${taxPeriod === 'Q3' ? 'bg-purple-600 text-white shadow-lg ring-2 ring-purple-300' : 'bg-white text-gray-700 border-2 border-purple-200 hover:border-purple-400'}`}>
                    Q3<span className='block text-[10px] font-normal mt-0.5'>9 Months</span>
                  </button>
                  <button type='button' onClick={() => setTaxPeriod('Q4')} className={`px-3 py-2 rounded-lg font-semibold text-sm transition-all cursor-pointer ${taxPeriod === 'Q4' ? 'bg-purple-600 text-white shadow-lg ring-2 ring-purple-300' : 'bg-white text-gray-700 border-2 border-purple-200 hover:border-purple-400'}`}>
                    Q4<span className='block text-[10px] font-normal mt-0.5'>12 Months</span>
                  </button>
                </div>
              </div>
          ...
          <div className='border-t border-gray-200 p-3 md:p-4 bg-gray-50 flex flex-col md:flex-row justify-between items-center gap-3 flex-shrink-0'>
            <div className='flex gap-2 md:gap-3 w-full md:w-auto'>
              ...
              <button type='submit' disabled={isSubmitting} className='flex-1 md:flex-none px-6 md:px-8 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:shadow-lg font-semibold transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed'>
                {isSubmitting ? (
                  <>
